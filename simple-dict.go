...
func query(trans_type, word string) {
	client := &http.Client{}
	request := DictRequest{TransType: trans_type, Source: word}
...
func main() {
	if len(os.Args) != 3 {
		fmt.Fprintf(os.Stderr, `usage:simpleDict WORD example:simpleDict en2zh hello`)
		os.Exit(1)
	}
	trans_type, word := os.Args[1], os.Args[2]
	query(trans_type, word)
}
